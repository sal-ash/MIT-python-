...
def getWordScore(word, n):
    ...
    y = len(word)
    for i in x:
        x[i] *= SCRABBLE_LETTER_VALUES[i] 
    if y == n: 
        score = (sum(x.values()) * y) + 50
    else:
        score = (sum(x.values()) * y)
    return score
    ...
    * The hand finishes when there are no more unused letters or the user
    ...
    while calculateHandlen(hand) > 0:
        ...
        # Otherwise (the input is not a single period):
            ...
            if isValidWord(x, hand, wordList) is False: 
            ...
            # Otherwise (the word is valid):
                ...
                total_score += score
                print('"' + x + '" earned ' + str(score) + ' points. Total: ' + str(total_score) + ' points')
                print()
                # Update the hand 
...
def playGame(wordList):
    ...
    while True:
        ...
        if x == 'n':
            z = dealHand(HAND_SIZE)
            playHand(z, wordList, HAND_SIZE)
        elif x == 'r':
            try:
                playHand(z, wordList, HAND_SIZE)
            except NameError:
